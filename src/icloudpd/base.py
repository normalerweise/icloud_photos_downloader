...
def _process_all_users_new_arch(
    global_config: GlobalConfig,
    user_configs: Sequence[UserConfig],
    logger: logging.Logger,
    shared_status_exchange: StatusExchange,
) -> int:
    """Process all user configs using the new download architecture (EXPERIMENTAL)."""
    ...
    for user_config in user_configs:
        logger.info(f"[new-arch] Processing user: {user_config.username}")
        ...
        for provider in global_config.password_providers:
            if provider == PasswordProvider.PARAMETER:
                ...
                password_providers_dict[provider] = (
                    partial(get_password_from_webui, logger, shared_status_exchange),
                    partial(update_password_status_in_webui, shared_status_exchange),
                )
        ...
        try:
            ...
            logger.exception(f"[new-arch] Authentication failed for {user_config.username}")
            return 1
        ...
        if not user_config.directory:
            logger.error("[new-arch] --directory is required")
            return 1
        ...
        logger.info("[new-arch] Sync complete.")
...
def _process_all_users_once(
    ...
    if global_config.new_arch:
        return _process_all_users_new_arch(
            global_config, user_configs, logger, shared_status_exchange
        )
