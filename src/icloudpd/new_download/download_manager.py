...
from .constants import DOWNLOAD_TIMEOUT, MAX_CONCURRENT_DOWNLOADS, RETRY_ATTEMPTS, RETRY_DELAY
...
class DownloadManager:
    ...
    def __init__(self, file_manager: FileManager):
        ...
        self.session.headers.update({"User-Agent": "icloudpd/1.0"})
        ...
        for attempt in range(RETRY_ATTEMPTS):
            try:
                ...
                if attempt < RETRY_ATTEMPTS - 1:
                    logger.warning(
                        f"Download failed for asset {asset_id} version {version}, attempt {attempt + 1}/{RETRY_ATTEMPTS}"
                    )
                    time.sleep(RETRY_DELAY * (2**attempt))  # Exponential backoff

            except Exception as e:
                logger.error(
                    f"Download error for asset {asset_id} version {version}, attempt {attempt + 1}: {e}"
                )
                if attempt < RETRY_ATTEMPTS - 1:
    ...
    def _download_from_url(self, url: str, file_path: Path, asset_id, version) -> bool:
        ...
        try:
            ...
            success = self.file_manager.save_file_from_stream(file_path, response.raw)
